...
function FilterRecords(page) {
    ...
    $("#tbodyArtists").html("");
    c.AjaxCall("AdminAPI/GetUserEvents", $.param(data), "GET", true, function (d) {
        CreateTable(d);
        Paginate(d);
    });
}
...
function CreateTable(data) {
    ...
    for (var i = 0; i < data.Items.length; i++) {
        ...
        + "<td>" + dateFormat(data.Items[i].Datetime_Local, "mm/dd/yyyy, h:MM:ss TT", "Standard") + "</td>"

        + "<td>" + data.Items[i].ArtistName + "</td>"
        + "<td>" + data.Items[i].VenueName + "</td>"

        + "<td><input type='button' class='btn btn-info btn-sm Remove' value='Remove Tracking' data-userid='" + data.Items[i].UserID + "' data-tourdateid='" + data.Items[i].TourDateID + "' />"
        + "</tr>";
    }
