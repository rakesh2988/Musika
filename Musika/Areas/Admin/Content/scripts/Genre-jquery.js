...
function FilterRecords(page) {
    pageNumber = page;
    var c = new Common();
    var data = c.getValues("#form-search-Genre", "");
    data["Pageindex"] = page;
    data["Pagesize"] = 30;
    ...
    $("#tbodyArtists").html("");
    c.AjaxCall("AdminAPI/GetGenre", $.param(data), "GET", true, function (d) {
        CreateTable(d);
        Paginate(d);
    });
}
...
function CreateTable(data) {
    ...
    for (var i = 0; i < data.Items.length; i++) {
        ...
        + "<td>" + data.Items[i].UserCount + "</td>"
        + "<td>" + data.Items[i].EventCount + "</td>"

        + "<td><input type='button' class='btn btn-info btn-sm view' value='View/Edit' data-genreid='" + data.Items[i].GenreID + "' />"
        + "<input type='button' class='btn btn-info btn-sm Delete' value='Delete' data-genreid='" + data.Items[i].GenreID +  "' style='margin-left:5px;' />"
        + "</td>"
        + "</tr>";
    }
...
function BindEvents() {
    ...
    $(".view").click(function () {
        ...
        cmn.AjaxCall("AdminAPI/GetGenreByID?ID=" + $(this).data("genreid"), {}, "GET", true, function (d) {
            console.log(d);
            $("#myModal").modal("show");
    ...
    $(".Delete").click(function () {
        DeleteGenre(this, $(this).data("genreid"));
    });
...
function DeleteGenre(btn, ID) {
    var c = new Common();
    c.AjaxCall("AdminAPI/DeleteGenre/" + ID, {}, "DELETE", true, function (d) {
        if (d.Status) {
...
function UpdateGenre(btn) {
    ...
    if (c.validate("form-Genre")) {
        ...
        c.AjaxCall("AdminAPI/updateGenre", JSON.stringify(data), "POST", true, function (d) {
            if (d.Status) {
                $('#myModal').modal("hide");
                c.ShowMessage(d.RetMessage, "success");
                FilterRecords(pageNumber);
